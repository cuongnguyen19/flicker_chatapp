...
const initialState: ArchivedChat = {
    currentId: null,
    conversations: [],
...
const archivedChatSlice = createSlice({
    name: "archivedChat",
    initialState,
    reducers: {
        setState: (state, action: PayloadAction<Partial<ArchivedChat>>) => {
        ...
        leaveConversation: (
            ...
            if (conversations.length > 0 && state.currentId !== null) {
                if (conversations.length > index && index > -1) {
                    action.payload.pushFunction(`/archivedChat/${conversations[index].id}`);
                } else action.payload.pushFunction(`/archivedChat/${conversations[conversations.length - 1].id}`);
            } else action.payload.pushFunction("/archivedChat");
        },
        addConversation: (
            state,
            action: PayloadAction<{
...
const searchConversationsAsyncAction = createAsyncThunk(
    "ArchivedChat/SearchConversations",
    async (
...
const getConversationMediaAsyncAction = createAsyncThunk(
    "ArchivedChat/GetConversationMedia",
    async (
        data: { messageApi: MessageInstance; conversationId: number; page?: number; size?: number },
        thunkAPI
    ) => {
...
const getConversationDocsAsyncAction = createAsyncThunk(
    "ArchivedChat/GetConversationDocs",
    async (
        data: { messageApi: MessageInstance; conversationId: number; page?: number; size?: number },
        thunkAPI
    ) => {
...
const markMessageAsSeenAsyncAction = createAsyncThunk(
    "ArchivedChat/MarkMessageAsSeen",
...
export const {
    ...
    concealConversation,
    unconcealConversation,
} = archivedChatSlice.actions;
export default archivedChatSlice.reducer;
