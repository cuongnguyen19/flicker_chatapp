...
import { searchArchivedMessages } from "@/shared/APIs/messageAPI";
...
const searchMessageModal = ({ conversation, user, open, onCancel, messageApi }: Props) => {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (query === "") {
        ...
        try {
          const response = await searchArchivedMessages(conversation.id, query);
  ...
  const loadFunc = async () => {
    ...
    const response = await searchArchivedMessages(conversation.id, query, page);
    response.content.forEach((m: any) => (m.id = m.id + "draft"));
