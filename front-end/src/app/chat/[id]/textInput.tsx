...
import {
  getConversationLockMessage,
  checkHiddenConversation, setConversationLockMessage,
  setConversationNotification, getConversationNotification
...
const TextInput = ({ params, conversations, conversationId, user, scrollToBottom, messageApi, dispatch }: Props) => {
          ...
          if(data) {
            if(data.lockMessage) {
              client.publish({
                destination: `/app/send/${conversationId}/${user.id}`,
                body: JSON.stringify({
                  content: "® This message is locked. Please right-click to reveal it. ®",
                  lockedContent: trimmedMessage,
                  locked: data.lockMessage,
                ...
                />)}
                  ...
                  <button
                      onClick={async (event) => {
                        ...
                        try {
                          const response = await setConversationLockMessage(
                              data.id,
                              !data.lockMessage
                          );
                          dispatch(
                              changeConversationLockMessage({
                                ...
                                lockMessage: response.lockMessage ? true : false,
                              })
                          );
                          messageApi.success(
                              response.lockMessage ? "Lock message" : "Unlock message"
                          );
                        } catch (e: any) {
                          messageApi.error(e.message);
                        }
                  ...
                  >
                    {data.lockMessage ? (
                        ...
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="icon icon-tabler icon-tabler-unlock"
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                            strokeWidth="1.5"
