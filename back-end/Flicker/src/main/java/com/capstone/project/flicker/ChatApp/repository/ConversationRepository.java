...
public interface ConversationRepository extends JpaRepository<Conversation, Long> {
    ...
    @Query(value = "SELECT c.* FROM conversation c WHERE c.conversation_id IN (SELECT ac.conversation_id FROM archived_conversation ac WHERE ac.user_id = :userId) ORDER BY c.updated_at DESC", nativeQuery = true)
    Set<Conversation> findArchivedConversationsForUserAfterLeft(@Param("userId") Long userId);
